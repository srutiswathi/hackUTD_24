...
st.set_page_config(page_title="🚗 Vehicle Analysis Dashboard", layout="wide")
...
def load_csv_from_ipfs(cid):
    ...
    if response.status_code == 200:
        # Use io.StringIO for handling response text as a file-like object
...
def encode_categorical_columns(data, categorical_columns):
    ...
    for column in categorical_columns:
        if data[column].dtype == 'object':  # Ensure it's a string/categorical column
            data[column] = encoder.fit_transform(data[column].fillna('Unknown'))
    return data

# Normalize columns
def normalize_column(column):
    if column.dtype in [np.float64, np.int64]:  # Only normalize numeric columns
...
def compute_group_scores(data, groups):
    for group_name, group_info in groups.items():
        ...
        if not valid_columns:
            st.warning(f"No valid columns found for {group_name}. Skipping.")
            continue
        
        # Normalize columns
        normalized_data = pd.DataFrame()
        for col in valid_columns:
            if group_name == "Sustainability" and col == "co2":  # Invert normalization for CO2
                normalized_data[col] = 1 - normalize_column(data[col])
            elif group_name == "Cost" and col in ["fuelCost08", "fuelCostA08"]:  # Higher cost is worse
                normalized_data[col] = 1 - normalize_column(data[col])
            else:
                normalized_data[col] = normalize_column(data[col])
        
        # Calculate weighted score
        data[f"{group_name} Score"] = sum(normalized_data[col] * weights[col] for col in valid_columns)
        
    return data

# Define groups and weights
groups = {
    "Sustainability": {
